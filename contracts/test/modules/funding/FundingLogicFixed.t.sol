...
contract MockRLDCore is IRLDCore {
    ...
    function createMarket(
        MarketAddresses calldata,
        MarketConfig calldata
    ) external override returns (MarketId) {
        return MarketId.wrap(0);
    }
    function isValidMarket(MarketId) external view override returns (bool) {
        return true;
    }
    function getMarketState(
        MarketId
    ) external view override returns (MarketState memory) {
        return MarketState(0, 0, 0);
    }
    ...
    function lock(bytes calldata) external override returns (bytes memory) {
        return "";
    }
    ...
    function getPendingRiskUpdate(
        MarketId
    ) external view override returns (PendingRiskUpdate memory) {
        return PendingRiskUpdate(0, 0, 0, 0, 0, 0, bytes32(0), 0, false);
    }
}
...
contract MockRateOracle is IRLDOracle {
    ...
    function setIndexPrice(uint256 p) external {
        price = p;
    }
