...
contract FixedYieldBond is Script {
    ...
    function run() external {
        ...
        uint256 currentRate = vm.envOr("CURRENT_RATE", uint256(0.10e18));     // Default 10%
        uint256 utilization = vm.envOr("UTILIZATION", uint256(0.90e18));      // Default 90%
        ...
        bytes32 salt = keccak256(abi.encodePacked("fixed-yield-", block.timestamp));
        ...
        if (!skipTwamm) {
            ...
            (address currency0, address currency1) = positionToken < collateralToken 
            ...
            // with the updated TwammBrokerModule pricing logic
            ...
            console.log("Duration:", durationDays, "days");
            console.log("");
            console.log("The TWAMM order will sell wRLP linearly over", durationDays, "days.");
            ...
            console.log("Set SKIP_TWAMM=false to enable TWAMM order submission");
            console.log("");
            console.log("========================================");
    ...
    function _calculateGamma(uint256 rate, uint256 durationYears) internal pure returns (uint256) {
