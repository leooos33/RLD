...
contract TestBrokerExecutorLP is Script {
    ...
    function run() external {
        ...
        uint256 expectedTokenId = IPositionManager(V4_POSITION_MANAGER).nextTokenId();
        ...
        calls[0] = BrokerExecutor.Call({
            target: broker,
            data: abi.encodeWithSignature("withdrawCollateral(address,uint256)", executorAddr, waUsdcAmount)
        });
        
        // 2. Withdraw wRLP from broker to executor
        calls[1] = BrokerExecutor.Call({
            target: broker,
            data: abi.encodeWithSignature("withdrawPositionToken(address,uint256)", executorAddr, wrlpAmount)
        });
        
        // 3. Approve Permit2 for waUSDC
        calls[2] = BrokerExecutor.Call({
            target: waUSDC,
            data: abi.encodeWithSignature("approve(address,uint256)", PERMIT2, type(uint256).max)
        });
        ...
        calls[4] = BrokerExecutor.Call({
            target: PERMIT2,
            data: abi.encodeWithSignature("approve(address,address,uint160,uint48)", 
                waUSDC, V4_POSITION_MANAGER, type(uint160).max, uint48(block.timestamp + 1 hours))
        });
        ...
        calls[5] = BrokerExecutor.Call({
            target: PERMIT2,
            data: abi.encodeWithSignature("approve(address,address,uint160,uint48)",
                positionToken, V4_POSITION_MANAGER, type(uint160).max, uint48(block.timestamp + 1 hours))
        });
        ...
        calls[7] = BrokerExecutor.Call({
            ...
            data: abi.encodeWithSignature("transferFrom(address,address,uint256)", 
                executorAddr, broker, expectedTokenId)
        });
        
        // 9. Register LP for NAV calculation
        calls[8] = BrokerExecutor.Call({
            target: broker,
            data: abi.encodeWithSignature("setActiveV4Position(uint256)", expectedTokenId)
        });
        ...
        bytes32 ethSignedHash = executor.getEthSignedMessageHash(broker, nonce);
        ...
        address nftOwner = IERC721(V4_POSITION_MANAGER).ownerOf(expectedTokenId);
        console.log("");
