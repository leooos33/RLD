...
contract ExtensiveTWAMMTest is Script {
    ...
    function run() external {
        ...
        // Trigger execution
        console.log("Triggering executeTWAMMOrders...");
    ...
    function _testOrder(
        ...
        vm.broadcast(userKey);
        (bytes32 orderId, ITWAMM.OrderKey memory orderKey) = twamm.submitOrder(
    ...
    function _queryAllOrders(uint256 userKey) internal {
        ...
        for (uint256 i = 0; i < results.length; i++) {
            ...
            ITWAMM.OrderKey memory orderKey = ITWAMM.OrderKey({
                owner: user,
                expiration: uint160(results[i].expiration),
                zeroForOne: true
            });
