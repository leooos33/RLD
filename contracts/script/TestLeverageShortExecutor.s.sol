...
contract TestLeverageShortExecutor is Script, StdCheats {
    ...
    function run() external {
        ...
        address broker = PrimeBrokerFactory(brokerFactory).createBroker(
            keccak256(abi.encode(block.timestamp, userC, "leverage-short"))
        );
        ...
        uint256 targetDebt = LeverageShortExecutor(executor).calculateOptimalDebt(
            initialCollateral,
            targetLTV,
            wRLPPriceE6
        );
        ...
        LeverageShortExecutor(executor).executeLeverageShort(
            broker,
            marketId,
            waUSDC,
            wRLP,
